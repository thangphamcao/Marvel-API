import { Fragment } from 'react';

function Footer() {
    return (
        <Fragment>
            <footer className=" bg-black bg-stone-950 mt-2 w-full ">
                <div className=" mx-auto p-4 md:py-8">
                    <div className="sm:flex sm:items-center sm:justify-between">
                        <a href="http://marvel.com" className="flex items-center mb-4 sm:mb-0">
                            <img
                                src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/63/Marvel_Studios_logo.svg/2560px-Marvel_Studios_logo.svg.png"
                                className="h-8 mr-3"
                                alt="Logo"
                            />
                            <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">
                                MARVEL COMIC FAKE
                            </span>
                            s
                        </a>
                        <ul className="flex flex-wrap items-center mb-6 text-sm font-medium text-gray-500 sm:mb-0 dark:text-gray-400">
                            <li>
                                <a href="#home" className="mr-4 hover:underline md:mr-6 ">
                                    About
                                </a>
                            </li>
                            <li>
                                <a href="#home" className="mr-4 hover:underline md:mr-6">
                                    Privacy Policy
                                </a>
                            </li>
                            <li>
                                <a href="#home" className="mr-4 hover:underline md:mr-6 ">
                                    Licensing
                                </a>
                            </li>
                            <li>
                                <a href="#home" className="hover:underline">
                                    Contact
                                </a>
                            </li>
                        </ul>
                    </div>
                    <hr className="my-6 border-gray-200 sm:mx-auto dark:border-gray-700 lg:my-8" />
                    <span className="block text-sm text-gray-500 sm:text-center dark:text-gray-400">
                        <a href="http://marvel.com" className="hover:underline">
                            Data provided by Marvel. © 2014 Marvel
                        </a>
                    </span>
                </div>
            </footer>
        </Fragment>
    );
}

export default Footer;
